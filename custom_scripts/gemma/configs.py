...
class RpaConfigs:
    ...
    def validate_inputs(
        ...
        if kv_cache.shape != expected_kv_cache_shape:
            raise ValueError(
                f"Expected kv_cache.shape={kv_cache.shape} to be equal to expected_kv_cache_shape={expected_kv_cache_shape}. "
                f"Model: num_kv_heads={self.model.num_kv_heads}, head_dim={self.model.head_dim}. "
                f"Aligned: num_kv_heads_x2={self.aligned_num_kv_heads_x2}, head_dim={self.aligned_head_dim}. "
                f"Mode={self.mode}"
            )
